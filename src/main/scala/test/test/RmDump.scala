...
object RmDump {
  ...
  def dumpIntoRm() = {
    ...
    dataToWrite.rdd.collect.foreach(record => {
      ...
      ON (
      '${record.getAs[String]("IVRS_PROJECT_ID")}' IN t.IVRS_PROJECT_ID 
      AND '${record.getAs[String]("IVRS_PROTOCOL_NUMBER")}' IN t.IVRS_PROTOCOL_NUMBER
      AND '${record.getAs[String]("IVRS_PATIENT_ID")}' IN t.IVRS_PATIENT_ID 
      AND '${record.getAs[String]("IVRS_COUNTRY")}' IN t.IVRS_COUNTRY
      )
